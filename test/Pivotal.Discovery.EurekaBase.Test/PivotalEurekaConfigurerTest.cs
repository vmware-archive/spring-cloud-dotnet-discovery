...
'eureka': {
    ...
    'instance': {
        ...
        [Fact]
        public void UpdateConfiguration_WithVCAPEnvVariables_DirectNoRoute_ConfiguresEurekaDiscovery_Correctly()
        {
            var vcap_application = @"
{
    'limits': {
...
'eureka': {
    ...
    'instance': {
        ...
        'secureHealthCheckUrl':'secureHealthCheckUrl'   
            ...
            Environment.SetEnvironmentVariable("CF_INSTANCE_INDEX", "1");
            Environment.SetEnvironmentVariable("CF_INSTANCE_GUID", "ac923014-93a5-4aee-b934-a043b241868b");
            Environment.SetEnvironmentVariable("CF_INSTANCE_INTERNAL_IP", "8.6.7.5");
            Environment.SetEnvironmentVariable("CF_INSTANCE_PORT", "309");
            var path = TestHelpers.CreateTempFile(appsettings);
            string directory = Path.GetDirectoryName(path);
            string fileName = Path.GetFileName(path);
            ConfigurationBuilder configurationBuilder = new ConfigurationBuilder();
            ...
            Assert.Equal("direct", ro.RegistrationMethod);
            Assert.Equal("8.6.7.5:instance_id", ro.InstanceId);
            Assert.Equal("foo", ro.AppName);
            Assert.Equal("appGroup", ro.AppGroupName);
            Assert.True(ro.IsInstanceEnabledOnInit);
            Assert.Equal(309, ro.NonSecurePort);
            Assert.Equal("8.6.7.5", ro.HostName);
            Assert.Equal(309, ro.SecurePort);
            Assert.True(ro.IsNonSecurePortEnabled);
            Assert.True(ro.SecurePortEnabled);
            Assert.Equal(100, ro.LeaseExpirationDurationInSeconds);
            Assert.Equal(100, ro.LeaseRenewalIntervalInSeconds);
        ...
        [Fact]
        public void UpdateConfiguration_WithVCAPEnvVariables_DirectWithRoute_ConfiguresEurekaDiscovery_Correctly()
        {
            var vcap_application = @"
{
    'limits': {
